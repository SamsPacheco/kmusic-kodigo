...
export const Recents = () => {
  ...
  return (
    <>
    {
        ...
        library.map( (recent, index) => <article className='h-[90px] xl:h-[75px] bg-[#31303b] flex items-center justify-start gap-2 xl:gap-3 rounded-md' key={index}>
        ...
        recents.map( (recent, index) => <article className='h-[70px] bg-[#31303b] flex items-center justify-start gap-2 xl:gap-3 rounded-md' key={index}>
            <figure className='h-full'>
            <img src={recent.picture_url} alt="" className='w-full h-full object-contain rounded-md' />
            </figure>
