  ...
  return (
    <main className='rounded-md lg:bg-gradient-to-t from-[#121212] via-[#191919] to-[#262f59] hidden lg:block h-screen  overflow-y-scroll'>
      ...
      <section className='flex items-end gap-5 p-8'>
        <figure className='w-[250px] rounded-lg'>
        ...
        <div>
          ...
          <h1 className='my-3 text-7xl font-[1000] tracking-tighter'>Liked Songs</h1>
      ...
      <section className='p-8 bg-gradient-to-t from-[#070707] via-[#101010] to-[#0f1325]/70 h-screen'>
        <section className='flex justify-between items-center mb-3'>
          ...
          <article className='flex items-center gap-4'>
            ...
            <i class="uil uil-arrow-circle-down text-[35px] text-gray-400"></i>
          </article>

          {/* search - date added list */}
          <article className='space-x-5 flex items-center text-[#d4d3d3]'>
            <i class="uil uil-search text-[20px]"></i>
            <span className='space-x-2'>Date added <i class="uil uil-list-ul text-[23px"></i></span>
          </article>
        </section>
        ...
        <section>
          <table class="min-w-full table-auto">
            ...
            <tbody class="text-sm">
              ...
              {
                musicList.map((music, index) => <tr className='border-b border-gray-800 hover:bg-gray-800'>
                  <td class="px-4 py-3">{index + 1}</td>
                  <td class="px-4 py-3 flex items-center gap-3">
                    <img src={music.photo_url} alt="cover" class="w-10 h-10 rounded" />
                    <div>
                      <p class="font-semibold text-white">{music.title}</p>
                      <p class="text-gray-400 text-xs">{music.artis}</p>
                    </div>
                  </td>
                  <td class="px-4 py-3 text-gray-300">{music.album}</td>
                  <td class="px-4 py-3 text-gray-300">{music.dateAdded}</td>
                  <td class="px-4 py-3 text-center text-gray-300">2:59</td>
                </tr>)
              }
